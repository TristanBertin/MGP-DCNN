...
total_time = shape[-2]
training_points = int(0.7 * total_time)
...
if peaks_selection == True:
    idx_peaks = np.argsort(yy_train[:,0])[-3:]
...
optimizer = torch.optim.Adam([
    {'params': model.parameters()},], lr=0.02)
...
new_index = np.array([np.arange(i,len_covar,5) for i in range(5)]).reshape(-1)
